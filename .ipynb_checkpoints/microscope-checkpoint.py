...
def cv_train_test(Dataset, 
    ...
    for fold, (train_index, test_index) in enumerate(cv.split(Dataset.df)):
        ...
        if feat_selection=="wilcox":
            print('Starting feature selection using wilcoxon')
            p_vals = stats.mannwhitneyu(df_train.iloc[np.where(y_train==0)[0], :], df_train.iloc[np.where(y_train==1)[0], :])[1]
            feats_to_keep = list(compress(df_train.columns, np.where(p_vals<feat_selection_pval_thresh)[0]))
            X_train = df_train.loc[:,feats_to_keep].values
            X_test = df_test.loc[:, feats_to_keep].values 
        elif feat_selection=="ttest":
            print('Starting feature selection using t-test')
            p_vals = stats.ttest_ind(df_train.iloc[np.where(y_train==0)[0], :], df_train.iloc[np.where(y_train==1)[0], :])[1]
            feats_to_keep = list(compress(df_train.columns, np.where(p_vals<feat_selection_pval_thresh)[0]))
            X_train = df_train.loc[:,feats_to_keep].values
            X_test = df_test.loc[:, feats_to_keep].values 
        else:
...
# ================================
def plot_cv_roc(cv_res, plot_individual_folds=True, out_f=None):
    fprs, tprs, aucs = [], [], []
...
def plot_pred_prob_by_labels(cv_res, out_f=None):
